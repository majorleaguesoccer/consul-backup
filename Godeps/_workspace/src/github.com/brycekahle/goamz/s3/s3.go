...
func (b *Bucket) PutBucket(perm ACL) (http.Header, error) {
...
func (b *Bucket) GetResponseWithHeaders(path string, headers map[string][]string) (resp *http.Response, err error) {
	...
	if u.RawQuery != "" {
		req.params, err = url.ParseQuery(u.RawQuery)
	}
...
func (b *Bucket) Put(path string, data []byte, contType string, perm ACL, options Options) (string, error) {
	body := bytes.NewBuffer(data)
	header, err := b.PutReader(path, body, int64(len(data)), contType, perm, options)
	if err != nil {
		return "", err
	}
	return header.Get("x-amz-version-id"), nil
}
...
// from r until EOF.
func (b *Bucket) PutReader(path string, r io.Reader, length int64, contType string, perm ACL, options Options) (http.Header, error) {
	headers := map[string][]string{
...
func (s3 *S3) query(req *request, resp interface{}) (http.Header, error) {
	err := s3.prepare(req)
	if err != nil {
		return nil, err
	}
